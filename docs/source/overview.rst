...
- **Zarr v3 only for write:** This version of hdmf-zarr writes zarr v3 format only. Zarr v2 folders can only be read in read-only mode with the :py:class:`~hdmf_zarr.nwb_v2.NWBZarrV2IO` class. Zarr v2 folders can be converted to v3 using the :py:meth:`~hdmf_zarr.nwb_v2.NWBZarrV2IO.convert_to_v3` function.

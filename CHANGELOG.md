...
* **Requirements Changes:** Bumped minimum required Python version to 3.12 and Zarr-Python dependency to `>=3.4.0` due to backwards-incompatible changes in Zarr v3 structured data types and bug fixes in Zarr-Python 3.4.0. Also bumped `hdmf` to `>=6.2.0`, `numpy` to `>=2.0.0`, and `pynwb` to `>=4.2.0`, and removed the `numcodecs<0.16.0` upper bound. @oruebel @rly [#373](https://github.com/hdmf-dev/hdmf-zarr/pull/373) [#382](https://github.com/hdmf-dev/hdmf-zarr/pull/382) [#402](https://github.com/hdmf-dev/hdmf-zarr/pull/402)

...
### Changed
- Adopted the unified Zarr v3 storage convention shared with Zindi/LINDI ([#335](https://github.com/hdmf-dev/hdmf-zarr/issues/335)). Links are stored in a `_LINKS` attribute on the parent group, dataset types in `_DTYPE`, `_SCALAR`, and `_REFERENCE_FIELDS`, references in attributes as `{"_REFERENCE": {"source", "path"}}`, and references in datasets as plain target path strings. `ZarrReference` drops the unused `object_id` and `source_object_id` keys, and `NaN`/`Infinity` attribute values are encoded as strings. The legacy `zarr_link` and `zarr_dtype` attributes are still read for Zarr v2 files via `ZarrV2IO`. A Zarr v3 store generated by Zindi from an HDF5 NWB file can now be read directly with `NWBZarrIO`. @bendichter [#336](https://github.com/hdmf-dev/hdmf-zarr/pull/336)
